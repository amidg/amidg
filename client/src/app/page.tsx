...
export default async function HomePage() {
  ...
  const STRAPI_URL = 'http://amidg_blog:1337';
  ...
  console.log(introBlock)

  return (
    <div className="flex flex-col items-center justify-center gap-2">
      <div className="flex flex-col justify-center w-full max-w-2xl md:mt-[5rem] p-4 md:p-0 text-light-text dark:text-violet-50">
        <div className="max-w-3xl w-full">
          <div className="flex flex-col items-center md:flex-row md:items-start md:space-x-6">
            <div className="mb-4 md:mb-0">
              <Avatar className="h-24 w-24 md:h-28 md:w-28 hover:scale-105 transition ease-in-out duration-200">
                {introBlock?.avatar?.url ? (
                  <>
                    {console.log(
                      "Avatar URL:",
                      getStrapiMedia(introBlock.avatar)
                    )}
                    <AvatarImage
                      src={getStrapiMedia(introBlock.avatar) || undefined}
                      alt={introBlock?.name || "Avatar"}
                    />
                  </>
                ) : (
          ...
          <div className="mt-16 max-w-2xl">
            <h1 className="text-baseline font-bold">
              {introBlock?.aboutTitle}
            </h1>
